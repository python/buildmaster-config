...
def is_failing_builder(builds):
    ...
    for build in builds:
        if not build["complete"]:
            continue
        if build["results"] == 2:  # 2 indicates a failed build
            failing_streak += 1
            first_failing_build = build
            continue
        elif build["results"] == 0:  # 0 indicates a successful build
...
def create_revert_pr(commit_sha, builder, failing_build):
    ...
    try:
        # Get the main repository and your fork
...
async def process_builder(session, builder, first_failing_build):
    try:
        # Get the change request for the first failing build
